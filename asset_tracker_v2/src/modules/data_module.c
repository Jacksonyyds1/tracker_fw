...
static void data_encode(void)
{
	...
	if (grant_send(CLOUD_LOCATION, &coneval, override)) {
		...
		extern struct cloud_data_cfg copy_cfg;
		if( copy_cfg.no_data.wifi == 1){
			LOG_ERR("Manu Stubbing this out since wifi not asked for %d",copy_cfg.no_data.wifi);
			err = -ENODATA;
		}
		switch (err) {
		case 0:
...
static void on_all_states(struct data_msg_data *msg)
{
	...
	if (IS_EVENT(msg, sensor, SENSOR_EVT_FUEL_GAUGE_READY)) {
		...
		LOG_DBG("SENSOR_EVT_FUEL_GAUGE_READY: %d", new_battery_data.bat);
...
#if 0	
	// static uint16_t culvert_batt = 3000;
	// if (IS_EVENT(msg, sensor, SENSOR_EVT_FUEL_GAUGE_NOT_SUPPORTED)) {
	// 	culvert_batt = culvert_batt<4000?culvert_batt+1:3000;
	// 	struct cloud_data_battery new_battery_data = {
	// 		.bat = culvert_batt, //msg->module.sensor.data.bat.battery_level,
	// 		.bat_ts = k_uptime_get(),//REVISIT
	// 		.queued = true
	// 	};
	// 	LOG_DBG("SENSOR_EVT_FUEL_GAUGE_NOT_SUPPORTED: %d", new_battery_data.bat);
	// 	cloud_codec_populate_bat_buffer(bat_buf, &new_battery_data,
	// 					&head_bat_buf,
	// 					ARRAY_SIZE(bat_buf));
	// 	requested_data_status_set(APP_DATA_BATTERY);
		...
		if (1) {
			...
			cloud_location.wifi_access_points.ts = msg->module.location.data.cloud_location.timestamp;
			cloud_location.wifi_access_points.queued = true;
#define NUM_FAKE_SSIDS 7
			uint8_t fake_ssid_culvert[][NUM_FAKE_SSIDS] = {
									{0x9c,0x3d,0xcf,0x3e,0x93,0xb6},
									{0x9e,0x3d,0xcf,0x3e,0x93,0xb7},
									{0x78,0x8c,0xb5,0x3b,0x84,0xd6},
									{0x38,0xa0,0x67,0x84,0xf5,0xa5},
									{0x38,0xa0,0x67,0x84,0xf5,0xa4},
									{0x78,0x8c,0xb5,0x3b,0x8b,0xa6},
									{0x7e,0x8c,0xb5,0x3b,0x8b,0xa6}
								};
			uint8_t fake_rssis_culvert[] = {-59,-59,-66,-69,-69,-70,-72};

			uint8_t fake_ssid_home[][NUM_FAKE_SSIDS] = {
									{0x7e,0x8f,0xe0,0xae,0x10,0x90},
									{0xa2,0x3d,0xcf,0x7f,0xa0,0xa2},   
									{0xcc,0x40,0xd0,0xf1,0x4f,0x58},   
									{0xa0,0x04,0x60,0xc4,0x8d,0x17},   
									{0xd8,0x07,0xb6,0x54,0x90,0xf6},   
									{0x80,0xab,0x4d,0xbf,0x92,0x04},   
									{0xd8,0x07,0xb6,0x54,0xa3,0x6e}
								};
			uint8_t fake_rssis_home[] = {-67,-69,-75,-77,-82,-82,-84};

			for ( int i = 0 ; i < NUM_FAKE_SSIDS;i++){
				memcpy(&cloud_location.wifi_access_points.ap_info[i].mac,
					 &fake_ssid_home[i],
					 6);
				cloud_location.wifi_access_points.ap_info[i].mac_length = 6;
				cloud_location.wifi_access_points.ap_info[i].rssi = fake_rssis_home[i];
			}
			cloud_location.wifi_access_points.cnt = NUM_FAKE_SSIDS;
		}
		cloud_location.ts = k_uptime_get();//REVISIT
