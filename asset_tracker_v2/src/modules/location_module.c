...
bool waiting_for_ssids = false;
static void ssid_wait_timeout_handler(struct k_timer *timer);
/* Application module message queue. */
K_TIMER_DEFINE(ssid_wait_timeout, ssid_wait_timeout_handler, NULL);
static void ssid_wait_timeout_handler(struct k_timer *timer)
{
	waiting_for_ssids = false;
	inactive_send();
}
static void search_start(void)
{
	...
	LOG_ERR("MHR says copy_cfg.no_data.wifi is %d\n",copy_cfg.no_data.wifi);
	extern int64_t last_ssids_received_ts;
	LOG_ERR("MHR says location was received %dms ago\n",k_uptime_get() - last_ssids_received_ts);
	/* Set default location configuration configured at compile time */
	location_config_defaults_set(&config, 0, NULL);

	if( copy_cfg.no_data.wifi == 0){
		LOG_WRN("Waiting 60s for SSIDs to trickle in...");
		waiting_for_ssids = true;
		SEND_EVENT(location, LOCATION_MODULE_EVT_ACTIVE);
		k_timer_start(&ssid_wait_timeout,K_SECONDS(60),K_SECONDS(0));
	}
	if( copy_cfg.no_data.gnss == 1){
		LOG_WRN("Not looking for GNSS...");
		return;
	}
	 ...
	 * that are enabled
	 */
	/* MHR OVERRIDE THIS EVERY TIME SO WE STUB OUT THE WIFI */
	// if (copy_cfg.no_data.neighbor_cell || copy_cfg.no_data.gnss || copy_cfg.no_data.wifi) {
	...
	if ( methods_count == 0){
		LOG_DBG("Not invoking location library, nothing for it to do\n");
	}else{
...
static void send_cloud_location_update(const struct location_data_cloud *cloud_location_info)
{
	...
	if (!copy_cfg.no_data.wifi &&
		cloud_location_info->wifi_data != NULL) {
		...
		LOG_ERR("MANU SAYS NO WIFI FOR YOU!!!");
	}
#endif
