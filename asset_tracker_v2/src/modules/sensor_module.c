...
static void environmental_data_get(void)
{
	...
	sensor_module_event->data.sensors.bsec_air_quality = counter++;
	 ...
	 */
	static int counter = 0;
	LOG_ERR("MHR adding an uptime counter %d", counter);
	sensor_module_event = new_sensor_module_event();
    uint32_t rr = nrfx_reset_reason_get();
    LOG_ERR("MHR RESET REASON is %lx\n",rr);

	__ASSERT(sensor_module_event, "Not enough heap left to allocate event");

	sensor_module_event->data.sensors.timestamp = k_uptime_get();
	sensor_module_event->data.sensors.temperature = 123;
	sensor_module_event->data.sensors.humidity = 456;
	sensor_module_event->data.sensors.pressure = rr;
	sensor_module_event->data.sensors.bsec_air_quality = counter++;
	sensor_module_event->type = SENSOR_EVT_ENVIRONMENTAL_DATA_READY;
