...
static struct spi_config spi_cfg = {
    .frequency = 4000000,
    .operation = SPI_OP_MODE_MASTER | SPI_WORD_SET(8) | SPI_LINES_SINGLE | SPI_MODE_CPOL | SPI_MODE_CPHA | SPI_TRANSFER_MSB,
    .slave = 0,
    .cs = {
...
static const struct device *spidev;// = DEVICE_DT_GET(DT_NODELABEL(spi2));



#if DEBUG_SPI_BUFFERS
//////////////////////////////////////////////////////////
// dbg_print_buf()
static void dbg_print_buf(char *name, const struct spi_buf_set set)
{
    for (int ib=0; ib < set.count; ib++) {
        LOG_DBG("%s[%d] (%08X) len = %d", name, ib, (uint32_t)(set.buffers[ib].buf), set.buffers[ib].len);
...
static int da_spi_read_data(da_rsp_t *rsp, wifi_msg_t *wifi_msg)
{
    ...
    uint32_t addr = ((rsp->buf_addr & 0xFF) << 24)\
                     + ((rsp->buf_addr & 0xFF00) << 8)\
                     + ((rsp->buf_addr & 0xFF0000) >> 8)\
...
{
    da_header_t header = {
        .addr_type = ((RESP_ADDR & 0xFF) << 24)\
                     + ((RESP_ADDR & 0xFF00) << 8)\
                     + ((RESP_ADDR & 0xFF0000) >> 8)\
...
static void da_resp_work_fn()
{
    ...
    if (wifi_on_rx_cb != NULL) {
        // Note that its expected that if the callback wants to consume
        // the message, it will call wifi_spi_recv() and wifi_msg_free() 
        // to free the memory
        wifi_on_rx_cb(&msg, wifi_on_rx_cb_userData);
    }
...
int wifi_spi_init(void) {
    ...
    k_work_queue_start(&da_resp_work_q, dialog_stack, K_THREAD_STACK_SIZEOF(dialog_stack), CONFIG_SYSTEM_WORKQUEUE_PRIORITY, NULL);
...
// wifi_spi_send
//
// Write an AT or ESC command to the DA16200. The DA should
// only process one at/esc cmd at a time.  This is
// critical to be able to match a response to a request.  So 
// this call can block if there is another request pending.
//
//  @param data - pointer to buffer containing the AT command
//  @param timeout - timeout for the write
