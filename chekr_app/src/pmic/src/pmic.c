...
void batt_info_work_handler(struct k_work *work)
{
    ...
    //modem_send_command(MESSAGE_TYPE_BATTERY_LEVEL, data, sizeof(float), NULL);
}
...
void pmic_int_work_handler(struct k_work *work)
{
    ...
    if (buf[0] & PMIC_CHARGE2_STATUS_VBUS_DETECTED_MASK) {
        ...
        if (reg & 0x4) {
            ...
            mfd_npm1300_reg_write(mfd, PMIC_BUCKCTRL_BASE_ADDR, PMIC_BUCKCTRL0_VOLTAGE_REG, reg);  // disable pulldowns - buck0 
        }
        ...
        mfd_npm1300_reg_write(mfd, PMIC_BUCKCTRL_BASE_ADDR, PMIC_BUCKCTRL0_VOLTAGE_REG, reg);  // discharge buck0 wth pulldowns
    ...
    switch (pwr_switch) {
        case PMIC_SWITCH_VSYS:
            if (newState) {
                ...
                for(int i = 0; i < 10; i++) {
                    //LOG_DBG("LDO_STATE: buf[0]=0x%02X  PMIC_LDO_STATUS_LDO1_MASK=0x%02X   (buf[0] & PMIC_LDO_STATUS_LDO1_MASK)=0x%02X", buf[0], PMIC_LDO_STATUS_LDO1_MASK, (buf[0] & PMIC_LDO_STATUS_LDO1_MASK));
                    if (!(buf[0] & PMIC_LDO_STATUS_LDO1_MASK)) {
                        return 0;
                    }
