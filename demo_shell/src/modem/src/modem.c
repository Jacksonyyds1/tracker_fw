...
int modem_send_command(modem_message_type_t type, uint8_t handle, uint8_t* data, uint16_t dataLen) {
#if (CONFIG_USE_UART_TO_NRF9160)
#else
    return modem_spi_send_command(type, handle, data, dataLen);
#endif
}
