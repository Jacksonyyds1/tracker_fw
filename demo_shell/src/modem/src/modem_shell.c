...
void do_modem_send(const struct shell *sh, size_t argc, char **argv)
{
    ...
    modem_send_command(MESSAGE_TYPE_AT, 1, argv[1], strlen(argv[1]));
...
void do_modem_send_json(const struct shell *sh, size_t argc, char **argv)
{
    uint8_t buf[256];
    shell_print(sh, "Sending to modem...\n");
    modem_send_command(MESSAGE_TYPE_JSON, 1, argv[1], strlen(argv[1]));
    shell_print(sh, "Modem returned: %s\n", buf);
}
...
void do_send_ssid_json(const struct shell *sh, size_t argc, char **argv)
{
    ...
    // send to spi
    modem_send_command(MESSAGE_TYPE_JSON, 1, retStr, strlen(retStr));

    //free(retStr);  // should free this, its important, but since this is just a test I can avoid it
                    // shell_print doesnt print immidiately and if you free it now, it crashes.
}

void do_send_raw(const struct shell *sh, size_t argc, char **argv)
{
    if (argc <= 1) { shell_print(sh, "usage: send_raw <string_to_send> \n"); return; }
...
SHELL_STATIC_SUBCMD_SET_CREATE(
    ...
    SHELL_CMD(send_ssids, NULL, "send a command fake SSID array to the modem", do_send_ssid_json),
