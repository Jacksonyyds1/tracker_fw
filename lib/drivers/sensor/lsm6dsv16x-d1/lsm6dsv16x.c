...
LOG_MODULE_REGISTER(LSM6DSV16X_D1, CONFIG_SENSOR_LOG_LEVEL);
...
int lsm6dsv16x_accel_odr_set(const struct device *dev, uint16_t freq)
{
	int odr;
...
int lsm6dsv16x_accel_range_set(const struct device *dev, int32_t range)
{
	int fs;
	struct lsm6dsv16x_data *data = dev->data;
				...
				const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_SLOPE_TH:
		data->sleep_thresh = CLAMP(val->val1, 0, 7);
		float thresholds[] = {7.8125, 15.625, 31.25, 62.5, 125, 250, 250, 250 };
		LOG_INF("Sleep threshold set to %.2f", thresholds[data->sleep_thresh]);
		lsm6dsv16x_inactivity_ths_t inactivity_ths;
		lsm6dsv16x_read_reg(ctx, LSM6DSV16X_INACTIVITY_THS, (uint8_t *)&inactivity_ths, 1);
		inactivity_ths.inact_ths = data->sleep_thresh;
		lsm6dsv16x_write_reg(ctx, LSM6DSV16X_INACTIVITY_THS, (uint8_t *)&inactivity_ths, 1);
		break;
	case SENSOR_ATTR_SLOPE_DUR:
		data->sleep_dur = CLAMP(val->val1, 0, 15);
		lsm6dsv16x_act_wkup_time_windows_t time;
		time.shock = 0;
		time.quiet = data->sleep_dur;
		LOG_INF("Sleep duration now approx %d secs", data->sleep_dur == 0 ? 1 : data->sleep_dur * 34);
		lsm6dsv16x_act_wkup_time_windows_set(ctx, time);
		break;
#endif
	default:
...
int lsm6dsv16x_gyro_odr_set(const struct device *dev, uint16_t freq)
{
	int odr;
...
int lsm6dsv16x_gyro_range_set(const struct device *dev, int32_t range)
{
	int fs;
	struct lsm6dsv16x_data *data = dev->data;
				...
				struct sensor_value *val)
{
	...
	switch((int)attr)
	{
	case SENSOR_ATTR_SLEEP_STATE:
		val->val1 = data->sleep_state;
		val->val2 = 0;
		break;
	case SENSOR_ATTR_SLOPE_TH:
		lsm6dsv16x_inactivity_ths_t inactivity_ths;
		lsm6dsv16x_read_reg(ctx, LSM6DSV16X_INACTIVITY_THS, (uint8_t *)&inactivity_ths, 1);
		val->val1 = data->sleep_thresh;
		val->val2 = 0;
		break;
	case SENSOR_ATTR_SLOPE_DUR:
		lsm6dsv16x_act_wkup_time_windows_t wkup;
		lsm6dsv16x_act_wkup_time_windows_get(ctx, &wkup);
		val->val1 = wkup.quiet;
		val->val2 = 0;
		break;
	default:
...
static int lsm6dsv16x_attr_set(const struct device *dev,
			    ...
			    const struct sensor_value *val)
{
#if defined(CONFIG_LSM6DSV16X_D1_SENSORHUB)
	struct lsm6dsv16x_data *data = dev->data;
#endif /* CONFIG_LSM6DSV16X_D1_SENSORHUB */
...
#if CONFIG_LSM6DSV16X_D1_ENABLE_MOTION
	.attr_get = lsm6dsv16x_attr_get,
#endif
#if CONFIG_LSM6DSV16X_D1_TRIGGER
...
{
#ifdef CONFIG_LSM6DSV16X_D1_TRIGGER
	const struct lsm6dsv16x_config *cfg = dev->config;
#endif
	struct lsm6dsv16x_data *data = dev->data;
...
#if CONFIG_LSM6DSV16X_D1_ENABLE_MOTION
	data->sleep_thresh = CONFIG_LSM6DSV16X_D1_SLEEP_THRESHOLD;
	data->sleep_dur = CONFIG_LSM6DSV16X_D1_SLEEP_DURATION;
#endif
...
#ifdef CONFIG_LSM6DSV16X_D1_SENSORHUB
...
#warning "LSM6DSV16X_D1 driver enabled without any devices"
#endif

/*
 * Device creation macro, shared by LSM6DSV16X_D1_DEFINE_SPI() and
 * LSM6DSV16X_D1_DEFINE_I2C().
 */

#define LSM6DSV16X_D1_DEVICE_INIT(inst)					\
...
#ifdef CONFIG_LSM6DSV16X_D1_TRIGGER
#define LSM6DSV16X_D1_CFG_IRQ(inst)					\
	.trig_enabled = true,						\
...
#else
#define LSM6DSV16X_D1_CFG_IRQ(inst)
#endif /* CONFIG_LSM6DSV16X_D1_TRIGGER */

#define LSM6DSV16X_D1_SPI_OP  (SPI_WORD_SET(8) |			\
			 SPI_OP_MODE_MASTER |				\
			 SPI_MODE_CPOL |				\
			 SPI_MODE_CPHA)					\

#define LSM6DSV16X_D1_CONFIG_COMMON(inst)				\
	...
	IF_ENABLED(UTIL_OR(DT_INST_NODE_HAS_PROP(inst, int1_gpios),	\
		   ...
		   (LSM6DSV16X_D1_CFG_IRQ(inst)))

#define LSM6DSV16X_D1_CONFIG_SPI(inst)					\
	{								\
		STMEMSC_CTX_SPI(&lsm6dsv16x_config_##inst.stmemsc_cfg),	\
		.stmemsc_cfg = {					\
			.spi = SPI_DT_SPEC_INST_GET(inst,		\
					   LSM6DSV16X_D1_SPI_OP,	\
					   0),				\
		},							\
		LSM6DSV16X_D1_CONFIG_COMMON(inst)			\
	}
...
#define LSM6DSV16X_D1_CONFIG_I2C(inst)					\
	{								\
...
#define LSM6DSV16X_D1_DEFINE(inst)						\
	static struct lsm6dsv16x_data lsm6dsv16x_data_##inst;			\
	static const struct lsm6dsv16x_config lsm6dsv16x_config_##inst =	\
		COND_CODE_1(DT_INST_ON_BUS(inst, spi),				\
			(LSM6DSV16X_D1_CONFIG_SPI(inst)),			\
			(LSM6DSV16X_D1_CONFIG_I2C(inst)));			\
	LSM6DSV16X_D1_DEVICE_INIT(inst)

DT_INST_FOREACH_STATUS_OKAY(LSM6DSV16X_D1_DEFINE)
