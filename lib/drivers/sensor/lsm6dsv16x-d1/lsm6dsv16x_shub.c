...
{
	return lsm6dsv16x_read_reg(ctx, LSM6DSV16X_D1_STATUS_MASTER_MAINPAGE, (uint8_t *)val, 1);
}

/*
 * LIS2MDL magn device specific part
 */
#ifdef CONFIG_LSM6DSV16X_D1_EXT_LIS2MDL
...
#endif /* CONFIG_LSM6DSV16X_D1_EXT_LIS2MDL */

/*
 * HTS221 humidity device specific part
 */
#ifdef CONFIG_LSM6DSV16X_D1_EXT_HTS221
...
#endif /* CONFIG_LSM6DSV16X_D1_EXT_HTS221 */

/*
 * LPS22HB baro/temp device specific part
 */
#ifdef CONFIG_LSM6DSV16X_D1_EXT_LPS22HB
...
#endif /* CONFIG_LSM6DSV16X_D1_EXT_LPS22HB */

/*
 * LPS22HH baro/temp device specific part
 */
#ifdef CONFIG_LSM6DSV16X_D1_EXT_LPS22HH
...
#endif /* CONFIG_LSM6DSV16X_D1_EXT_LPS22HH */

/*
 * LPS22DF baro/temp device specific part
 */
#ifdef CONFIG_LSM6DSV16X_D1_EXT_LPS22DF
...
#endif /* CONFIG_LSM6DSV16X_D1_EXT_LPS22DF */
...
static void lsm6dsv16x_shub_enable(const struct device *dev, uint8_t enable)
{
	...
	if (lsm6dsv16x_sh_master_set(ctx, enable) < 0) {
		...
		lsm6dsv16x_mem_bank_set(ctx, LSM6DSV16X_D1_MAIN_MEM_BANK);
		return;
	}
...
static int lsm6dsv16x_shub_set_data_channel(const struct device *dev)
{
	...
	lsm6dsv16x_sh_slave_connected_t aux = LSM6DSV16X_D1_SLV_0_1_2;
...
int lsm6dsv16x_shub_fetch_external_devs(const struct device *dev)
{
	...
	/* read data from external target */
	if (lsm6dsv16x_mem_bank_set(ctx, LSM6DSV16X_D1_SENSOR_HUB_MEM_BANK) < 0) {
	...
	for (n = 0; n < data->num_ext_dev; n++) {
		...
		if (lsm6dsv16x_read_reg(ctx, sp->sh_out_reg,
			...
			(void) lsm6dsv16x_mem_bank_set(ctx, LSM6DSV16X_D1_MAIN_MEM_BANK);
			return -EIO;
		}
	}

	return lsm6dsv16x_mem_bank_set(ctx, LSM6DSV16X_D1_MAIN_MEM_BANK);
}
...
int lsm6dsv16x_shub_init(const struct device *dev)
{
	 ...
	 */
	if (lsm6dsv16x_sh_write_mode_set(ctx, LSM6DSV16X_D1_ONLY_FIRST_CYCLE) < 0) {
		LOG_DBG("shub: error setting write once");
		return -EIO;
	}

	for (n = 0; n < ARRAY_SIZE(lsm6dsv16x_shub_slist); n++) {
		if (data->num_ext_dev >= LSM6DSV16X_D1_SHUB_MAX_NUM_TARGETS) {
			break;
		}
	...
	for (n = 0, regn = 0; n < data->num_ext_dev; n++) {
		sp = &lsm6dsv16x_shub_slist[data->shub_ext[n]];
		sp->sh_out_reg = LSM6DSV16X_D1_SENSOR_HUB_1 + regn;
