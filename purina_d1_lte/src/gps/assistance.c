...
LOG_MODULE_REGISTER(gnss_assist, 4);
...
int my_nrf_cloud_jwt_generate(uint32_t time_valid_s, char *const jwt_buf, size_t jwt_buf_sz)
{
	...
	char buf2[NRF_CLOUD_CLIENT_ID_MAX_LEN + 1];
	struct jwt_data jwt = {
		.audience = NULL,
	...
	if (IS_ENABLED(CONFIG_NRF_CLOUD_CLIENT_ID_SRC_INTERNAL_UUID)) {
		...
		// SMR - special - manipulate the response, the provisioning scripts use a different prefix than this returns
		// nrf_cloud_client_id_get will return something like nrf-351516178718623
		// our provisioning used dog- as the prefix, so we need to change the prefix to end up as dog-351516178718623
		memset(buf2, 0, sizeof(buf2));
		memcpy(buf2, CONFIG_PURINA_D1_LTE_CLIENT_ID_PREFIX, strlen(CONFIG_PURINA_D1_LTE_CLIENT_ID_PREFIX));
		char* dash = strstr(buf, "-");
		if (dash) {
			memcpy(buf2 + strlen(CONFIG_PURINA_D1_LTE_CLIENT_ID_PREFIX), dash + 1, strlen(dash + 1));
			jwt.subject = buf2;
			LOG_DBG("---------- JWT subject: %s", buf2);
		}
		else {
			jwt.subject = buf;
		}
...
static void get_pgps_data_work_fn(struct k_work *work)
{
	...
	err = my_nrf_cloud_jwt_generate(0, jwt_buf, sizeof(jwt_buf));
	if (err) {
		LOG_ERR("Failed to generate JWT, error: %d", err);
